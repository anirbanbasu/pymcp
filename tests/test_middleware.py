...
class TestStripUnknownArgumentsMiddleware:
    ...
    def test_strip_unknown_arguments(self, mcp_client: Client, caplog):
        ...
        assert hasattr(results, "content"), (
            "Expected results to have 'content' attribute"
        )
        assert hasattr(results, "structured_content"), (
            "Expected results to have 'structured_content' attribute"
        )
        assert "result" in results.structured_content, (
            "Expected 'structured_content' to have 'result' key"
        )
    ...
    def test_no_arguments_provided(self, mcp_client: Client, caplog):
        ...
        middleware_logs = [
            record for record in caplog.records if "Unknown arguments" in record.message
        ]
        assert len(middleware_logs) == 0, (
            "Expected no middleware logging when no arguments provided"
        )
    ...
    def test_only_valid_arguments(self, mcp_client: Client, caplog):
        ...
        assert len(unknown_arg_logs) == 0, (
            "Expected no unknown argument logging for valid args only"
        )
